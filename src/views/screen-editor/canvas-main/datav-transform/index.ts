...
// TODO
const handHelpLine = (coms, com) => {
  const { x: x1, y: y1, w: w1, h: h1 } = com.attr
  coms.forEach(co => {
    const { x, y, w, h } = co.attr
    if (Math.abs(y - y1) <= 2) {
      com.attr.y = y
    }
    else if (Math.abs(y / 2 - y1 / 2) <= 2) {
      com.attr.y = y + h / 2 - h1 / 2
    }
  })
}
