...
export default defineComponent({
  ...
  setup() {
    ...
    const dataFilters = computed(() => {
      const ids: any = apiDataConfig.value.pageFilters.map(m => m.id)
    ...
    const onUsedChange = (id: number | string, val: boolean) => {
    ...
    const removeFilter = (id: number | string) => {
      if (id) {
    ...
    const saveFilter = async (data: DataFilter) => {
      if (data.id) {
        ...
        apiDataConfig.value.pageFilters.push({ id: newId as any, enabled: true })
        newDataFilter.value = null
        refreshData()
      }
    }
