 ...
 * @LastEditTime : 2024-01-25 16:53:26
-->
<template>
...
const option = computed(() => {
  ...
  const opt = {
    ...
    plotOptions: {
      line: {
        marker: plotOptions.series.line.marker,
      },
      series: {
        ...plotOptions.series,
        dataLabels: {
    ...
    xAxis: {
      ...
      categories: dv_data.value?.[dv_field.value.y],
    },
    yAxis: {
      ...parseD3PublicConfig(yAxis),
    },
    series: dv_data.value?.[dv_field.value.x]?.map((item, idx) => {
      return {
        name: item || '',
        data: dv_data.value?.[dv_field.value.z]?.[idx] || [],
