 ...
 * @LastEditTime : 2024-01-29 19:05:13
-->
<template>
  ...
  >
    <el-tab-pane label="图形" tab="pie">
      ...
      >
        <g-field :level="2" label="阴影">
          <g-switch v-model="config.plotOptions.series.shadow" />
        </g-field>
        <g-field :level="2" label="折线属性">
          <g-input-number
            v-model="config.plotOptions.series.lineWidth"
            :step="1"
            suffix=""
            inline="inline"
            label="折线宽度"
          />
          <g-select
            v-model="config.plotOptions.series.dashStyle"
            :data="lineTypes"
            inline="inline"
            label="折线样式"
          />
        ...
        <g-field :level="2" label="连接属性" :is-flat="true">
          <g-switch v-model="config.plotOptions.series.connectNulls" inline label="连接空值" />
          <g-switch v-model="config.plotOptions.series.connectEnds" inline label="连接末端" />
        </g-field>
      </g-field-collapse>
      <g-field-collapse
        v-model="config.plotOptions.series.line.marker.enabled"
        :level="2"
        label="折线拐点"
        toggle
      >
        <g-field label="拐点" :level="2">
          <g-input-number
            v-model="config.plotOptions.series.line.marker.radius"
            :min="0"
            inline
            label="拐点半径"
          />
          <g-select
            v-model="config.plotOptions.series.line.marker.symbol"
            :data="iconTypes"
            inline="inline"
            label="拐点样式"
          />
    ...
    <el-tab-pane label="Y轴" tab="y">
      <g-field-collapse v-model="config.yAxis.visible" label="X轴" toggle>
...
const props = defineProps({
  com: {
    type: Object as PropType<D3Line>,
    required: true,
  },
})
...
const lineTypes = GlD3LineStyles
const iconTypes = GlHigtIcons
