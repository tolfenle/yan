...
<template>
  <chartGenerateConfig v-if="config.generate.configType === GenerateType.基础" :config="config" chart-type="none">
    ...
    <g-field :level="2" label-span="6" label="前缀">
      <g-input v-model="config.top.prefix" />
    </g-field>
  ...
  >
    ...
    <el-tab-pane label="TOP" tab="global">
      <g-field :level="2" label="是否显示">
        <g-switch v-model="config.top.show" label="单条数据配置请前往系列中配置" />
      </g-field>
...
import { ScrollRank, ScrollRankSeries } from './scroll-rank'
import { ElTabs, ElTabPane } from 'element-plus'

export default defineComponent({
  name: 'VScrollRankProp',
  components: {
    ElTabs,
    ElTabPane,
  },
